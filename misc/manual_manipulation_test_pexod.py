...
robotId = p.loadURDF("pexod_base_tmp.urdf",[0,0,.5], p.getQuaternionFromEuler([0.0, 0.0, 0.0]), useFixedBase=False)

debug_param = dict()
for joint in range(p.getNumJoints(robotId)):
    debug_param[joint] = p.addUserDebugParameter("joint {}".format(joint), -1.57, 1.57, 0)
