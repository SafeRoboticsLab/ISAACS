...
from simulators import RaceCarSingle5DEnv, PrintLogger, save_obj
...
def get_trajectories(
    env: RaceCarSingle5DEnv, vel_list, yaw_list, num_pts=5, T_rollout=150,
...
def visualize(
    env: RaceCarSingle5DEnv, policy: SAC, fig_path: str, vel_list: List[float],
    yaw_list: List[float], num_pts: int = 5, T_rollout: int = 150,
    end_criterion: str = "failure", subfigsz_x: float = 4.,
    subfigsz_y: float = 4., nx: int = 100, ny: int = 100,
    batch_size: int = 512, cmap: str = 'seismic', vmin: float = -.25,
    vmax: float = .25, alpha: float = 0.5, fontsize: int = 16,
    vel_scatter: bool = False, markersz: int = 40
):
  n_row = len(yaw_list)
  n_col = len(vel_list)
  ...
  trajectories, results = get_trajectories(
      env, vel_list, yaw_list, num_pts=num_pts, T_rollout=T_rollout,
...
def main(config_file):
  ...
  # Constructs solver.
  print("\n== Solver information ==")
  solver = NaiveRL(cfg.solver, cfg.train, cfg.arch, cfg.environment)
