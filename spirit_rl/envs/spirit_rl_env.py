...
class SpiritRLEnv(gym.Env):
    ...
    def test_step(self, action):
        ...
        spirit_old_joint_position = np.array(self.test_robot.get_joint_position(), dtype = np.float32)
        
        self.test_robot.apply_position(action)
        ...
        spirit_new_joint_position = np.array(self.test_robot.get_joint_position(), dtype = np.float32)
    ...
    def open_gui(self, client):
        p.configureDebugVisualizer(
            p.COV_ENABLE_GUI, 0, physicsClientId = client)
        p.configureDebugVisualizer(
            p.COV_ENABLE_SEGMENTATION_MARK_PREVIEW, 0, physicsClientId = client)
        p.configureDebugVisualizer(
            p.COV_ENABLE_DEPTH_BUFFER_PREVIEW, 0, physicsClientId = client)
        p.configureDebugVisualizer(
            p.COV_ENABLE_RGB_BUFFER_PREVIEW, 0, physicsClientId = client)
        p.configureDebugVisualizer(p.COV_ENABLE_RENDERING, 0)
        p.resetDebugVisualizerCamera(
            cameraDistance=1,
            cameraYaw=20,
            cameraPitch=-20,
            cameraTargetPosition=[1, -0.5, 0.8], physicsClientId = client)
    ...
    def rollout_reset(self):
        ...
        # self.open_gui(self.rollout_client)
    ...
    def rollout_step(self, action):
        ...
        spirit_old_joint_position = np.array(self.rollout_robot.get_joint_position(), dtype = np.float32)
        self.rollout_robot.apply_position(action)
        ...
        ob = np.array(spirit_observation, dtype = np.float32)
        ob_old = np.array(spirit_old_observation, dtype = np.float32)
        spirit_new_joint_position = np.array(self.rollout_robot.get_joint_position(), dtype = np.float32)
    ...
    def step(self, action, safety_action = False):
        ...
        spirit_old_joint_position = np.array(self.robot.get_joint_position(), dtype = np.float32)
        ...
        elif self.i > self.max_train_steps:
            done = True
        else:
            raise ValueError("invalid doneType")
    ...
    def simulate_one_trajectory(self, agent, **kwargs):
        ...
        with torch.no_grad():
            test_action, _ = agent.actor.sample(
                test_state.float().to(self.device))
            test_action = test_action.cpu().numpy()

        # take the current action, receive the next state
        test_state, r, done, info = self.test_step(test_action)

        for step in range(test_steps):
            test_action = agent.actor(test_state.float().to(self.device))
            test_action = test_action.detach().cpu().numpy()
            test_state, r, done, info = self.test_step(test_action)
            
            if info["g_x"] > 0: # not safe
                return -1
            elif info['l_x'] <=0 and info['g_x'] <= 0:
                return 1
        return 0

    def simulate_trajectories(self, agent, num_traj=100, **kwargs):
        return np.array([self.simulate_one_trajectory(agent) for i in range(num_traj)])